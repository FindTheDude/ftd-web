...
var FindTheDude = function(){
    ...
    var concurrencyFriends    = 1;
    var concurrencyPhotos     = 25;
    var concurrencyDetect     = 5;
        ...
        cv.readImage( photoFile , function(err, im){
          ...
          im.detectObject(cv.FACE_CASCADE, {}, function(err, faces){
            ...
            for (var i=0 ; i < len ; i++){
              ...
              var file      = fs.path.basename( photoFile , '.jpeg' ) + "_" + i + ".jpeg";
    ...
    var downloadPhotoQueue = async.queue( function( task, callback ){
        ...
        if( !fs.existsSync( imageFile  ) ){
          ...
          request(source).pipe(fs.createWriteStream( imageFile )).on('close', function( err ){
             ...
             //Detect Faces
             detectFacesQueue.push( { userId: userId , photoFile: imageFile , tagsFile: tagsFile } );
    ...
    var getPhotosForFriendQueue = async.queue( function( task, callback ){
        ...
        var userId   = task.userId;
        var friendId = task.friendId;
        var url    = userId + '/photos/?fields=source,tags';
        _queryFB( url , function( err , photos ){

            //Download the photos
            var len = photos.length;
            for( var i = 0 ; i < len ; i++ ){
                downloadPhotoQueue.push( { userId: userId , friendId: friendId , photo: photos[ i ] } );
            }
    ...
    this.prepare = function( userId , callback ){
        ...
        //Download My Photos
        getPhotosForFriendQueue.push( { userId: userId , friendId: userId } );
        ...
        _queryFB( userId + '/friends/' , function( err , friends ){
            ...
            for( var i = 0 ; i < len ; i++ ){
                getPhotosForFriendQueue.push( { userId: userId , friendId: friends[i].id } );
            }
